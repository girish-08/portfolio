# Girish Salunke - Portfolio Website 💼

Welcome to my personal portfolio website! This responsive web app showcases my projects, education, and skills, with smooth animations, dark mode, and contact form validation.

## 🌐 Live Demo
[Click here to view the live website](https://your-portfolio-link.com)

---

## 📌 Features

- ✅ Responsive design (mobile + desktop)
- 🍔 Hamburger menu with side navigation
- 🎯 Smooth scrolling navigation
- 📝 Contact form with real-time validation
- 🔥 Scroll animations for sections
- 🌗 Dark mode toggle with local storage
- 🧠 Skills progress animation
- 🎓 Project and education cards with entry animation
- 🧪 Project filtering by category
- 📅 Auto year update in footer

---

## 🛠️ Built With

- **HTML5**
- **CSS3**
- **JavaScript**
- **AOS** (Animate on Scroll)
- **Typed.js** for typing effect

---

## 📂 Project Structure

. ├── index.html ├── style.css ├── script.js ├── images/ │ └── (all portfolio images) ├── assets/ │ └── (optional fonts, icons) └── README.md

---

